# src/models/train_baseline_rf.py
...
def ensure_dirs(outputs_dir):
    for sub in ["metrics", "folds", "feature_importance", "models"]:
...
def run_spatial_cv(model, X, y, groups, n_splits=5):
...
def main():
    ...
    if args.spatial_block_deg is not None:
        ...
        folds_df, spatial_summary = run_spatial_cv(
            rf_cv, X_model, y, groups=groups, n_splits=args.n_splits
        )
        ...
        # Full-data fit for reusable model and importances
